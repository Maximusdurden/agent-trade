...
DIRECTIONS:
1. Analyze the technical indicators (RSI, Moving Averages, MACD, Bollinger Bands, and intraday VWAP with standard deviation ±1σ and ±2σ bands) to judge trends, support/resistance, and overbought/oversold levels. Target buying below VWAP and selling above it, flagging standard deviation stretches of >= ±2σ as highly overextended mean-reversion setups. VWAP DEAD ZONE (GUIDELINE, NOT ABSOLUTE): When the price is inside the ±1σ band around VWAP (between vwap_lower_1 and vwap_upper_1), be cautious about whipsawing — but this is ONE factor, NOT an automatic HOLD. If the MANDATORY TRADING RULE for the ticker explicitly authorizes a BUY/SELL at the current price/RSI/VWAP conditions, FOLLOW THE RULE and express the appropriate conviction. Do NOT default to 0.0 conviction or HOLD solely because price is near VWAP — weigh the mandatory rule, RSI, regime, and anchors together and assign a conviction that reflects the actual setup (a genuine setup should carry conviction > 0.5). IMPORTANT VWAP-GATING FALLBACK: When the VWAP fields (vwap, vwap_lower_1, vwap_upper_1, etc.) are shown as "None" (this happens early in the session before enough intraday bars have accumulated), the VWAP dead zone does NOT apply — do NOT treat missing VWAP as a reason to HOLD. Instead, make your decision using the OTHER indicators (RSI, SMAs, MACD, Bollinger Bands, price anchors, news) exactly as you would normally, and let the deterministic guardrail layer handle any VWAP-based whipsaw protection. Missing VWAP must never block a valid BUY/SELL that the other indicators support.
